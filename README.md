...
OllamaTUI can run as an MCP server, exposing its tools to other AI applications:
...
ollamatui --mcp --cd /path/to/project

# With specific approval policy
ollamatui --mcp --approval never
```

The MCP server exposes these tools:
- `bash` - Execute bash commands
- `file` - File operations (read, write, edit, list, search)
- `git` - Git operations
...
```json
{
  "mcpServers": {
    "ollamatui": {
      "command": "ollamatui",
      "args": ["--mcp", "--cd", "/path/to/your/project"]
    }
  }
}
```
